...
const gNumberInput = {
    ...
    switch (event.type) {
      case "change": {
        if (this.changeExpr(event)) {
          ...
          else item.value = $(item.getAttribute("delayPreference"))?.valueFromPreferences ?? item.defaultValue;
        }
  ...
  updateSpinnerDisabledState(item) {
    ...
    setItem(item, "increaseDisabled", value >= max || null);
    setItem(item, "decreaseDisabled", value <= min || null);
    function setItem(aItemOrId, aAttr, aVal) {
