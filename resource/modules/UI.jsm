...
this.UI = {
	...
	updateViewportRatio: function(arrangeGroups) {
		...
		if(!viewportRatio) {
			...
			// Assume the dimensions of the browser panel for the screen ratio, it's not perfect, but it'll do for now. 
			viewportRatio = gBrowser.tabpanels.clientWidth / gBrowser.tabpanels.clientHeight;
			...
			if(canRearrange && maxColumns * maxRows >= visiblePairs.length
				...
				visiblePairs = visiblePairs.sort((a, b) => {
					...
					return { item: pair.item, bounds: new Rect(
						safeBounds.left + (safeBounds.width - rowItems * cellWidth - (rowItems - 1) * gutter) / 2
